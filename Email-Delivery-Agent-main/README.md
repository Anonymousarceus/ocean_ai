...
Note: The backend LLM integration uses Google Gemini via the `google-generativeai` package. You can switch `GEMINI_MODEL` in your `.env` to a different supported model. Never commit your real `GEMINI_API_KEY` to version control; keep `.env` out of public repositories.
